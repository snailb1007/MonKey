...
|---------|-------------------|------------|----------------------|
| **Dual-Interface Transport Multiplexing** | Separates the paths reported in local research notes and keeps transport concerns isolated. | HIGH | Candidate design for `0xFF68` bulk and `0xFFFF` configuration paths; report sizes, permissions, and availability require raw capture confirmation on Monka. |
...
- [ ] **Static LCD Image Upload (`monkey lcd image <file>`)** — Candidate 128x128 RGB565 pipeline and 8 x 4096-byte transfer over reported `0xFF68`; validate transport and the <50ms target with raw captures and benchmarks first.
